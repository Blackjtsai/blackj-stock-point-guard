...
- **引用邊界**：下游程式（job prompts、其他文件）引用規格內容時，一律指向 `docs/design/` 的衍生文件（如 SDD.md），不得直接引用 `docs/requirements/` 來源文件的章節編號——來源文件會持續改版，章節編號不穩定（見 ADR-002）
...
- 每次排程執行必須依 `docs/design/SDD.md` 第 6.3 節報告欄位規範（A 總經底座 / B 個股籌碼表 / C 建議動作 / D 資金警語與回補提示）產出 Markdown 報告
- 報告檔名固定格式：`reports/{YYYY-MM-DD}/{HHMM}_{PRE|MID|POST}.md`
- 資料抓不到或多來源矛盾時，該欄位標示「資料未取得／來源不一致」，禁止用推測值或記憶中的舊數字填充。但「與既有參考值差很大」不等於「資料可疑」——先換權威來源查證再說（見 ADR-010）
- **個股報價主來源＝ TWSE MIS 官方 API**（Bash curl 批次 + python 解析，省 token 零幻覺），WebFetch Yahoo 為 fallback；日 K 前波高低用 TWSE STOCK_DAY／TPEx tradingStock（見 SDD 6.2、ADR-010）。籌碼數據不在此列，仍走個股頁（ADR-003）
