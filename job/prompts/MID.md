...
1. 依 `docs/design/SDD.md` 第 6.3 節欄位規範撰寫報告，C 段建議動作要明確寫「維持早盤建議」或「調整為：xxx（含原因）」。語氣依 6.3 節「語氣與敘事風格」規範撰寫，僅限包裝與呈現方式，不得影響數字的真實性。若某代號因 SDD 6.6 護盾續抱規則而被擋下「高位停利變現」，必須在該代號 C 段明確寫出「護盾續抱」與安全墊百分比，不能只寫「維持早盤建議」而不交代原因。**不需要自己另外組「延續數據表」**——只要 `last_price`／`limit_range` 有正確寫進 `reports/state.json`（見下方第 3 點），該表格會由 `job/append_continuity_table.py` 自動決定性附加，不假手你排版（見 ADR-005）。
